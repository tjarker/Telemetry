...
The radio connection is the pivot point of the telemetry system and a flexible way to pass commands and associated payloads both ways is needed. 
The nRF24L01+ module uses the Enhanced Shockburst $\copyright$ packet structure to transmit messages over the 2.4 GHz ISM band. Messages contain a data payload of up to 32 bytes, allowing us comfortably to transmit one CAN telemetry message per packet. Further, the package structure includes a preamble, address, packet and CRC bytes segments as shown in Figure \ref{fig:shockburst}. These are all generated automatically when a call to the radio write function is made, and therefore fall outside the scope of this project.   
\begin{figure}[h]
    \centering
